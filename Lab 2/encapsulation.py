...
class Subject:
    ...
    def assign_teacher(self, instructor):
...
def enroll_to_subject(student, subject):
    try:
        if student.student_id.isnumeric() and len(student.student_id) == 8:
            registered = Enrollment(student, subject)
            for regis in enrollment_list:
                if regis.student == student and regis.subject == subject:
                    return "Already Enrolled"
            enrollment_list.append(registered)
            return "Done"
        else:
            raise ValueError()
    except:
...
def search_enrollment_subject_student(subject, student):
    for registration in enrollment_list:
        if registration.subject == subject and registration.student == student:
            return registration
    ...
    try:
        ...
        for registration in enrollment_list:
            if registration.subject == subject:
                subject_student_list.append(registration)
        return subject_student_list
    except:
        return "Error"
...
def search_subject_that_student_enrolled(student):
    ...
    for registration in enrollment_list:
        if registration.student == student:
            student_subject_list.append(registration)
    if student_subject_list == None:
        return "Not Found"
    else:
...
def assign_grade(student, subject, grade):
    registration = search_enrollment_subject_student(subject, student)
    if registration == None:
        return "Not Found"
    elif registration.grade == None:
...
def get_no_of_student_enrolled(subject):
    subject_student_list = search_student_enroll_in_subject(subject)
    if subject_student_list == None:
        return "Not Found"
    else:
        return len(subject_student_list)
...
def get_student_record(student):
    student_subject_list = search_subject_that_student_enrolled(student)
    record = {}
    for registration in student_subject_list:
        record[registration.subject.subject_id] = registration.subject.subject_name, registration.grade
    return record
...
def get_student_GPS(student):
    ...
    total_credit = 0
    total_score = 0
    for subject_id, subject_grade in record.items():
        subject = search_subject_by_id(subject_id)
        total_credit += subject.credit
        grade = grade_to_count(subject_grade[1])
        score = grade * subject.credit
        total_score += score
    total_grade = f"{total_score/total_credit:.1f}"
    return total_grade
